...
function startTest() {
    ...
    //Create an endTest function to end the test and display results when the user finishes the text.
    function endTest() {
...
//         Here is the breakdown of above code: - endTime = new Date().getTime();: This line captures the current time when the test ends. - document.getElementById("userInput").readOnly = true;: Disables further input in the userInput textarea, preventing the user from typing after the test is complete.

// Time calculation: var timeElapsed = (endTime - startTime) / 1000;
...
// Calculating words per minute (WPM): var userTypedText = document.getElementById("userInput").value; var typedWords = userTypedText.split(/\s+/).filter(function (word) { return word !== "";}).length;
...
// Word Per Minute: if (timeElapsed !== 0 && !isNaN(typedWords)) { wpm = Math.round((typedWords / timeElapsed) * 60);}
